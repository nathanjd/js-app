module.exports = (grunt) ->
  grunt.initConfig
    ...
    copy:
      main:
        files: [
          { dest: 'compiled/', cwd: 'src', src: '*.html', expand: true }
          { dest: 'compiled/css/mocha.css', src: 'src/css/mocha.css' }
          {
            dest: 'compiled/js/lib/'
            cwd: 'src/js/lib'
            src: '**'
            flatten: true
            expand: true
      ...
      compile:
        options:
          ...
          processName: (filename) ->
            templateDir = grunt.config 'pkg.templateDir'
    ...
    s3:
      ...
      deploy:
        upload: [
          { src: 'compiled/**', rel: 'compiled', access: 'public-read' }
        ]
    watch:
      coffee:
        files: ['src/js/**/*.coffee']
  ...
  grunt.registerTask 'bucket', 'Generate bucket target file', (bucket) ->
    grunt.file.write 'src/js/bucket.coffee', 'define () -> "' + bucket + '"'

  # set bucket, compile and push, then set bucket back to dev
  grunt.registerTask 'deploy', 'Push to an s3 bucket', (bucket) ->
